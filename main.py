...
def main(ban):
    ...
    support_flags_sources, lastseen_sources, basic_stats = analysis_functions.stats_syntactic(ban)
    ...
    # Peer List Diversity
    pl_div_ips, max_announced_subnets, median_announced_subnets = analysis_functions.peer_list_diversity(ban, threshold=0.1)
    ...
    pl_sim_ips, overlap_df = analysis_functions.peer_list_similarity(ban, threshold=0.3)
    ...
    # ID:IP Anomalies
    sus_id_cluster_ips, sus_id_anomaly_ips, interection_ips, union_ips, id_count_dist, num_clusters = analysis_functions.node_ids(ban=ban)
    ...
    sus_short, sus_ping, sus_ts, all_latencies, conn_df = analysis_functions.connections(ban=ban, threshold=90, min_tss=10) 
    ...
    banned_ips, signature_only_ips = analysis_functions.ban_and_signature(ban=ban)
    ...
    sus_subnet_ips, lion_peers, median_subnet_peers = analysis_functions.subnets_asn_comb(ban=ban)
    ...
    # In-Degree analysis
    indegrees = pd.DataFrame(analysis_functions.indegree(ban))

    # Final Eval
    identified_ns_peers = analysis_functions.summarize_IPs_plot_overlap(ban, anomaly_dict)
    
    all_ns_peers = analysis_functions.save_ns_peers(identified_ns_peers)

    saturation_stats = analysis_functions.plot_anom_saturation(ban, total_anomaly_set=identified_ns_peers)

    reachable_peers = analysis_functions.reachable(ban, identified_ns_peers)

    ban_list_stats = analysis_functions.compare_to_banlist(all_ns_peers)

    pl_poison = analysis_functions.pl_poison_dist(ban, identified_ns_peers)

    analysis_functions.format_and_write_tex(ban, basic_stats, anomaly_dict, conn_df, all_latencies, indegrees, 
